...
Schedule <- R6::R6Class(
  ...
  private = list(
    ...
    validateProps = function(`dayOfMonth` = NULL, `dayOfWeek` = NULL, `hour` = NULL, `minute` = NULL, `month` = NULL) {
      ...
      if (!is.null(`minute`)) {
        .setPrimitiveProperty(typeList = list("numeric", "character"), propertyData = minute)
      }
      if (!is.null(`month`)) {
  ...
  public = list(
    ...
    #' @param minute The minute(s) of the day that the job will run. Allowed values are either &#x60;&#x60;[\&quot;*\&quot;]&#x60;&#x60; meaning every minute of the day or&#x60;&#x60;[0 ... 59]&#x60;&#x60;.
    ...
    initialize = function(`dayOfMonth` = NULL, `dayOfWeek` = NULL, `hour` = NULL, `minute` = NULL, `month` = NULL, validateParams = FALSE, ...) {
      local.optional.var <- list(...)
      if (validateParams) {
        lapply(list(`dayOfMonth`, `dayOfWeek`, `hour`, `minute`, `month`), function(param) {
    ...
    validate = function() {
      do.call(private$validateProps, list(dayOfMonth = self$`dayOfMonth`, dayOfWeek = self$`dayOfWeek`, hour = self$`hour`, minute = self$`minute`, month = self$`month`))
    },
      ...
      ',
        ...
        if (!is.null(self$`minute`)) {
          sprintf(
            ...
            [%s]
      ',
            paste(sapply(self$`minute`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox = TRUE, digits = NA)), collapse = ",")
          )
        },
    ...
    #' @param ScheduleJson A JSON encoded string representation of a class instance.
    #' @param validateParams An optional param for auto validating this object's parameters after deserialization. Default FALSE.
    fromJSON = function(ScheduleJson, validateParams = FALSE) {
      ScheduleObject <- jsonlite::fromJSON(ScheduleJson)
      self$`dayOfMonth` <- ApiClient$new()$deserializeObj(ScheduleObject$`dayOfMonth`, "array[AnyOfnumberstring]", loadNamespace("datarobot.apicore"))
      self$`dayOfWeek` <- ApiClient$new()$deserializeObj(ScheduleObject$`dayOfWeek`, "array[AnyOfnumberstring]", loadNamespace("datarobot.apicore"))
      self$`hour` <- ApiClient$new()$deserializeObj(ScheduleObject$`hour`, "array[AnyOfnumberstring]", loadNamespace("datarobot.apicore"))
      self$`minute` <- ApiClient$new()$deserializeObj(ScheduleObject$`minute`, "array[AnyOfnumberstring]", loadNamespace("datarobot.apicore"))
      self$`month` <- ApiClient$new()$deserializeObj(ScheduleObject$`month`, "array[AnyOfnumberstring]", loadNamespace("datarobot.apicore"))
