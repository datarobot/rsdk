...
GCPIntake <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param GCPIntakeJson A JSON encoded string representation of a class instance.
    #' @param validateParams An optional param for auto validating this object's parameters after deserialization. Default FALSE.
    fromJSON = function(GCPIntakeJson, validateParams = FALSE) {
      GCPIntakeObject <- jsonlite::fromJSON(GCPIntakeJson)
      self$`credentialId` <- GCPIntakeObject$`credentialId`
      self$`format` <- GCPIntakeObject$`format`
      self$`type` <- GCPIntakeObject$`type`
      self$`url` <- GCPIntakeObject$`url`
