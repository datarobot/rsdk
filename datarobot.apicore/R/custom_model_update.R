...
CustomModelUpdate <- R6::R6Class(
  ...
  public = list(
    ...
    validate = function() {
      props <- list(classLabels = self$`classLabels`, description = self$`description`, desiredMemory = self$`desiredMemory`, language = self$`language`, maximumMemory = self$`maximumMemory`, name = self$`name`, negativeClassLabel = self$`negativeClassLabel`, networkEgressPolicy = self$`networkEgressPolicy`, positiveClassLabel = self$`positiveClassLabel`, predictionThreshold = self$`predictionThreshold`, replicas = self$`replicas`, requiresHa = self$`requiresHa`, targetName = self$`targetName`)
