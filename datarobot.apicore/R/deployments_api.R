...
DeploymentsApi <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description Create deployment from recommended model
    #' Produces: "application/json"
    #'
    #' @details Create a deployment for a recommended model
    #' @param deploymentCreateFromProjectRecommendedModel \link{DeploymentCreateFromProjectRecommendedModel}.
    #' @param ... Optional. Additional named parameters to be passed downward.
    #' @return \link{DeploymentCreateResponse}
    #' @details Response status codes, messages, and headers:
    #' \itemize{
    #' \item **`200`** The model is already deployed.
    #' \itemize{
    #' }
    #' \item **`202`** Job submitted. The URL at the Location header can be used to track when the deployment is ready for predictions.
    #' \itemize{
    ...
    #' \dontrun{
    #' library(datarobot.apicore)
    #' deploymentCreateFromProjectRecommendedModel <- DeploymentCreateFromProjectRecommendedModel$new() # DeploymentCreateFromProjectRecommendedModel |
    #'
    #' api.instance <- DeploymentsApi$new()
    #' result <- api.instance$DeploymentsFromProjectRecommendedModelCreate(deploymentCreateFromProjectRecommendedModel=deploymentCreateFromProjectRecommendedModel)
    #' }
    DeploymentsFromProjectRecommendedModelCreate = function(deploymentCreateFromProjectRecommendedModel = NULL, ...) {
      apiResponse <- private$DeploymentsFromProjectRecommendedModelCreateWithHttpInfo(deploymentCreateFromProjectRecommendedModel, ...)
      resp <- apiResponse$response
      if (httr::status_code(resp) == 202) {
        # When the DataRobot Public API returns a 202, this means that
        # an asynchronous job or task has been started. The response will
    ...
    #' @param serviceHealth Enum < [unavailable, failing, warning, unknown, passing] > Filters deployments by their service health status.
    #' @param modelHealth Enum < [unavailable, failing, warning, unknown, passing] > Filters deployments by their model health status.
    #' @param accuracyHealth Enum < [unavailable, failing, warning, unknown, passing] > Filters deployments by their accuracy health status.
    #' @param role Enum < [USER, OWNER, DEMO_OBSERVER] > Filter deployments to only those that the authenticated user has the specified role for.
    #' @param status Enum < [launching, stopping, active, replacingModel, inactive, archived, errored] > Filters deployments by their status
    ...
    #' maxExplanations <- OneOfintegerstring$new() # OneOfintegerstring | The maximum number of prediction explanations to supply per row of the dataset. The default `maxExplanations` is `3` when `xemp` based explanations are requested.  The default `maxExplanations` is `all` (no limit) when `shap` based explanations are requested.
    #' maxCodes <- OneOfintegerstring$new() # OneOfintegerstring | Deprecated alias of `maxExplanations`.
  ...
  private = list(
    ...
    # A helper function to invoke the API operation `DeploymentsFromProjectRecommendedModelCreate`. This function is responsible for
    # validating request parameters, building the request, deserializing the response, and handling errors.
    DeploymentsFromProjectRecommendedModelCreateWithHttpInfo = function(deploymentCreateFromProjectRecommendedModel = NULL, ...) {
      args <- list(...)
      queryParams <- list()
      headerParams <- c()

      if (!missing(`deploymentCreateFromProjectRecommendedModel`) && isa(deploymentCreateFromProjectRecommendedModel, c("DeploymentCreateFromProjectRecommendedModel", "R6"))) {
        body <- `deploymentCreateFromProjectRecommendedModel`$toJSON()
      } else {
        stop("DeploymentsFromProjectRecommendedModelCreateWithHttpInfo requires parameter deploymentCreateFromProjectRecommendedModel to be of type DeploymentCreateFromProjectRecommendedModel.")
      }

      urlPath <- "/deployments/fromProjectRecommendedModel/"
