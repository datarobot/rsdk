...
AutopilotStart <- R6::R6Class(
  ...
  public = list(
    ...
    validate = function() {
      props <- list(autopilotClusterList = self$`autopilotClusterList`, blendBestModels = self$`blendBestModels`, considerBlendersInRecommendation = self$`considerBlendersInRecommendation`, featurelistId = self$`featurelistId`, mode = self$`mode`, prepareModelForDeployment = self$`prepareModelForDeployment`, runLeakageRemovedFeatureList = self$`runLeakageRemovedFeatureList`, scoringCodeOnly = self$`scoringCodeOnly`)
