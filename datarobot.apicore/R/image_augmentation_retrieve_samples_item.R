...
ImageAugmentationRetrieveSamplesItem <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`height` = NULL, `imageId` = NULL, `originalImageId` = NULL, `projectId` = NULL, `width` = NULL, validateParams = FALSE, ...) {
      ...
      requiredProps <- list(`height`, `imageId`, `originalImageId`, `projectId`, `width`)
      if (validateParams) {
    ...
    validate = function() {
      props <- list(height = self$`height`, imageId = self$`imageId`, originalImageId = self$`originalImageId`, projectId = self$`projectId`, width = self$`width`)
