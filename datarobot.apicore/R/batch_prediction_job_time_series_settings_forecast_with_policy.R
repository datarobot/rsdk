...
BatchPredictionJobTimeSeriesSettingsForecastWithPolicy <- R6::R6Class(
  ...
  private = list(
    ...
    validateProps = function(relaxKnownInAdvanceFeaturesCheck = NULL, type = NULL) {
      if (!is.null(`type`)) {
  ...
  public = list(
    ...
    initialize = function(`type` = NULL, `relaxKnownInAdvanceFeaturesCheck` = FALSE, validateParams = FALSE, ...) {
      local.optional.var <- list(...)
      requiredProps <- list(`type`)
      if (validateParams) {
        lapply(requiredProps, missing)
        private$validateProps(relaxKnownInAdvanceFeaturesCheck, type)
      }
    ...
    validate = function() {
      props <- list(relaxKnownInAdvanceFeaturesCheck = self$`relaxKnownInAdvanceFeaturesCheck`, type = self$`type`)
