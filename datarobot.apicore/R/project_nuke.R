...
ProjectNuke <- R6::R6Class(
  ...
  public = list(
    ...
    validate = function() {
      do.call(private$validateProps, list(creator = self$`creator`, deletedAfter = self$`deletedAfter`, deletedBefore = self$`deletedBefore`, limit = self$`limit`, offset = self$`offset`, organization = self$`organization`, projectIds = self$`projectIds`, searchFor = self$`searchFor`))
    },
