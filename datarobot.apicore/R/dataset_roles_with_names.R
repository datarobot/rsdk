...
DatasetRolesWithNames <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(`name` = NULL, `role` = NULL, `shareRecipientType` = NULL, `canShare` = FALSE, `canUseData` = NULL, validateParams = FALSE, ...) {
      ...
      requiredProps <- list(`name`, `role`, `shareRecipientType`)
      if (validateParams) {
    ...
    validate = function() {
      props <- list(canShare = self$`canShare`, canUseData = self$`canUseData`, name = self$`name`, role = self$`role`, shareRecipientType = self$`shareRecipientType`)
