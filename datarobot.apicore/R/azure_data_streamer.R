...
#' @docType class
#' @title AzureDataStreamer
#'
#' @description AzureDataStreamer Class
#'
...
AzureDataStreamer <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param AzureDataStreamerJson A JSON encoded string representation of a class instance.
    #' @param validateParams An optional param for auto validating this object's parameters after deserialization. Default FALSE.
    fromJSON = function(AzureDataStreamerJson, validateParams = FALSE) {
      AzureDataStreamerObject <- jsonlite::fromJSON(AzureDataStreamerJson)
      self$`credentialId` <- .setPrimitiveProperty(typeList = list("character"), propertyData = AzureDataStreamerObject$credentialId)
      self$`format` <- AzureDataStreamerObject$`format`
      self$`type` <- AzureDataStreamerObject$`type`
      self$`url` <- AzureDataStreamerObject$`url`
