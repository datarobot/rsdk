...
BatchPredictionJobUpdate <- R6::R6Class(
  ...
  public = list(
    ...
    validate = function() {
      do.call(private$validateProps, list(aborted = self$`aborted`, completed = self$`completed`, failedRows = self$`failedRows`, hidden = self$`hidden`, jobIntakeSize = self$`jobIntakeSize`, jobOutputSize = self$`jobOutputSize`, logs = self$`logs`, scoredRows = self$`scoredRows`, skippedRows = self$`skippedRows`, started = self$`started`, status = self$`status`))
    },
