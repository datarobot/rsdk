...
CustomModelCreate <- R6::R6Class(
  ...
  public = list(
    ...
    validate = function() {
      props <- list(calibratePredictions = self$`calibratePredictions`, classLabels = self$`classLabels`, customModelType = self$`customModelType`, description = self$`description`, desiredMemory = self$`desiredMemory`, language = self$`language`, maximumMemory = self$`maximumMemory`, name = self$`name`, negativeClassLabel = self$`negativeClassLabel`, networkEgressPolicy = self$`networkEgressPolicy`, positiveClassLabel = self$`positiveClassLabel`, predictionThreshold = self$`predictionThreshold`, replicas = self$`replicas`, requiresHa = self$`requiresHa`, supportsBinaryClassification = self$`supportsBinaryClassification`, supportsRegression = self$`supportsRegression`, targetName = self$`targetName`, targetType = self$`targetType`)
