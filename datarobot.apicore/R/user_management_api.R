...
UserManagementApi <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param orderBy Enum < [username, -username, userGroup, -userGroup, lastName, -lastName, firstName, -firstName, status, -status, expirationDate, -expirationDate] > The order that the results should be retrieved in. Prefix the attribute name with a dash to sort in descending order, e.g. &#x60;orderBy&#x3D;-username&#x60;.
    #' @param isActive Enum < [false, False, true, True] > If specified, filters results by account activation status.
    #' @param isAdmin Enum < [false, False, true, True] > If specified, filters results to admin users (system and org) or non-admin users.
    #' @param ... Optional. Additional named parameters to be passed downward.
    ...
    #' }
    GroupsUsersList = function(offset, limit, groupId, namePart = "", orderBy = NULL, isActive = NULL, isAdmin = NULL, ...) {
      apiResponse <- private$GroupsUsersListWithHttpInfo(offset, limit, groupId, namePart, orderBy, isActive, isAdmin, ...)
      resp <- apiResponse$response
      if (httr::status_code(resp) == 202) {
        # When the DataRobot Public API returns a 202, this means that
        # an asynchronous job or task has been started. The response will
  ...
  private = list(
    ...
    # validating request parameters, building the request, deserializing the response, and handling errors.
    GroupsUsersListWithHttpInfo = function(offset, limit, groupId, namePart = "", orderBy = NULL, isActive = NULL, isAdmin = NULL, ...) {
      args <- list(...)
      queryParams <- list()
      headerParams <- c()
