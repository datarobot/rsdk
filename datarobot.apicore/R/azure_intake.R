...
AzureIntake <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param AzureIntakeJson A JSON encoded string representation of a class instance.
    #' @param validateParams An optional param for auto validating this object's parameters after deserialization. Default FALSE.
    fromJSON = function(AzureIntakeJson, validateParams = FALSE) {
      AzureIntakeObject <- jsonlite::fromJSON(AzureIntakeJson)
      self$`credentialId` <- AzureIntakeObject$`credentialId`
      self$`format` <- AzureIntakeObject$`format`
      self$`type` <- AzureIntakeObject$`type`
      self$`url` <- AzureIntakeObject$`url`
